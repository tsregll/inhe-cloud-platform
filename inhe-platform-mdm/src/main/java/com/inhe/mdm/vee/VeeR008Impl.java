...
@Service
public class VeeR008Impl implements IRuleProcess{//用电量远远高于平均水平(平均值)
		...
		try {
			if("09".equals(type) || "10".equals(type)) {//用电量
				...
				if(params==null||params.size()!=2) {
					throw new Exception("R007 params invalid");
				}
				...
				String maxFlag = params.getJSONObject(1).getString("ignore");
                   if("N".equals(minFlag)&&"N".equals(maxFlag)) {
					...
					if (KwhList==null||KwhList.size()<times) {
						MdmVeeRuleResult res = new MdmVeeRuleResult("R008", 2, "Fail,Unable to verify (missing data)");
						result.add(res);
					}
					else {
						Double iNum = 0.0d;
			...
			{
				MdmVeeRuleResult res = new MdmVeeRuleResult("R008", 2, "Fail,This rule is not suitable");
				result.add(res);
			}
		}catch(Exception e) {
			e.printStackTrace();
	...
	private Boolean valid(MdmAmDevice device,Boolean passFlag, Date time,String refCode, String cmd, String data, String pass, String alarmFlag, Double currentValue,
		...
		MdmVeeRuleResult res = new MdmVeeRuleResult("R008", 0, "Success,in average level");
		if(Double.compare(currentValue,avg*quot)>0) {
			passFlag = false;
			res.setResult(1);
			res.setRemarks("Fail,much higher than average");
			if("Y".equals(alarmFlag)) {
				commonService.sendMsg("-000918",null, time, data, cmd, refCode);
			}
		}
		result.add(res);
		return passFlag;
