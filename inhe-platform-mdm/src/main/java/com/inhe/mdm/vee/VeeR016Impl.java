...
@Service
public class VeeR016Impl implements IRuleProcess{//最大需量与上个月比较
	...
	public Boolean check(MdmAmDevice device,JSONObject rule,JSONObject msgBody,JSONArray result) {
		...
		try {
			...
			if(params==null||params.size()!=2) {
				throw new Exception("R016 params invalid");
			}
			...
			if("05".equals(type)&&"04".equals(cmd.substring(0, 2))&&("6".equals(cmd.substring(cmd.length()-1,cmd.length()))||"9".equals(cmd.substring(cmd.length()-1,cmd.length())))) {//账单数据
				...
				Date before = DateUtil.rollByMonth(time, -1);
				Date sTime=null,eTime=null;
                eTime = DateUtil.rollSecond(simf.parse(DateUtil.dateToString(time, DateUtil.DATE_MONTH_LINE)+"-01 00:00:00"),-1);
				...
				MdmVeeRuleResult res = new MdmVeeRuleResult("R016", 2, "Fail,This rule is not suitable");
				result.add(res);
			}
	...
	private Boolean valid(Boolean passFlag, Date time,String refCode, String cmd, String data, String pass, String alarmFlag, Double currentValue,
		...
		MdmVeeRuleResult res = new MdmVeeRuleResult("R016", 0, "Success,value in range compared with last month");//成功,最大需量较上月符合预设范围
		if(lastValue.compareTo(-1d)!=0) {//有值
			...
			if("N".equals(minFlag)&&"N".equals(maxFlag)) {
				if(Double.compare(commonService.getVeeData(data), compareMax)>0 || Double.compare(commonService.getVeeData(data), compareMin)<0) {
					passFlag = false;
					res.setResult(1);
					res.setRemarks("Fail,this value not in range compared with last month("+compareMin+"-"+compareMax+")");
					if("Y".equals(alarmFlag)) {
						commonService.sendMsg("-000931",device, time, data, cmd,refCode);
					}
				}
