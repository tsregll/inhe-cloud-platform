...
@Service
public class VeeR007Impl implements IRuleProcess{//用电量远远低于平均水平(常量)
	@Autowired
	...
	public Boolean check(MdmAmDevice device,JSONObject rule, JSONObject msgBody,JSONArray result) {
		...
		try {
			...
			{
				MdmVeeRuleResult res = new MdmVeeRuleResult("R007", 2, "Fail,This rule is not suitable");
				result.add(res);
			}
		}catch(Exception e) {
			e.printStackTrace();
	...
	private Boolean valid(MdmAmDevice device,Boolean passFlag, Date time,String refCode, String cmd, String data, String pass, String alarmFlag, Double currentValue,
		...
		MdmVeeRuleResult res = new MdmVeeRuleResult("R007", 0, "Success,in range");
		if(Double.compare(currentValue,val*quot)<0) {
			passFlag = false;
			res.setResult(1);
			res.setRemarks("Fail,far below average (constant)");
			if("Y".equals(alarmFlag)) {
				commonService.sendMsg("-000917",device, time, data, cmd,refCode);
			}
		}
		result.add(res);
		return passFlag;
