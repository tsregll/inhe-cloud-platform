...
@Service
public class VeeR005Impl implements IRuleProcess{//本次值为0
...
//		type = commonService.parseType(dataType,cmd);
	...
	private Boolean valid(MdmAmDevice device, Boolean passFlag, Date time, String refCode, String cmd, String data,
	        String pass, String alarmFlag, Double currentValue,JSONArray result) throws Exception {
		MdmVeeRuleResult res = new MdmVeeRuleResult("R005", 0, "Success,this value!=0");
		if(currentValue.compareTo(0d)==0) {
			passFlag = false;
			if("Y".equals(alarmFlag)) {
				commonService.sendMsg("-000915", device, time, data, cmd, refCode);
			}
			res.setResult(1);
			res.setRemarks("Fail,this value=0");
		}
		result.add(res);
		return passFlag;
	}
