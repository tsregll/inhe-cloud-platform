...
@Service
public class VeeR015Impl implements IRuleProcess{//用电量与去年同个月比较
	@Autowired
	...
	public Boolean check(MdmAmDevice device,JSONObject rule, JSONObject msgBody,JSONArray result) {
		...
		try {
			...
			if("10".equals(type)) {//用电量
				...
				//取上一年用电量	
				qry = DateUtil.dateToString(DateUtil.rollYear(readTime2, -1), "yyyy-MM");
			...
			{
				MdmVeeRuleResult res = new MdmVeeRuleResult("R015", 2, "Fail,This rule is not suitable");
				result.add(res);
			}
		}catch(Exception e) {
			e.printStackTrace();
	...
	private Boolean valid(Boolean passFlag, Date time,String refCode, String cmd, String data, String pass, String alarmFlag, Double lastMonth,
		...
		MdmVeeRuleResult res = new MdmVeeRuleResult("R015", 0, "Success");
		if (lastMonth!=null) {
			...
			if("N".equals(minFlag)&&"N".equals(maxFlag)) {
				if(Double.compare(current, compareMax)>0 || Double.compare(current, compareMin)<0) {
					...
					if("Y".equals(alarmFlag)) {
						commonService.sendMsg("-000930",device, time, data, cmd, refCode);
					}
				}
				...
				if(Double.compare(current, compareMin)<0) {
					passFlag = false;
					res.setResult(1);
					res.setRemarks("Fail,this <minValue");
					if("Y".equals(alarmFlag)) {
						commonService.sendMsg("-000930",device, time, data, cmd, refCode);
					}
				}
				...
				if(Double.compare(current, compareMax)>0) {
					passFlag = false;
					res.setResult(1);
					res.setRemarks("Fail,this >maxValue");
					if("Y".equals(alarmFlag)) {
						commonService.sendMsg("-000930",device, time, data, cmd, refCode);
					}
				}
