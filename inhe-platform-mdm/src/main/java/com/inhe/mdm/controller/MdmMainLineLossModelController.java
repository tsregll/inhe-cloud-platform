...
*/ 
@RestController
@RequestMapping("/api/loss/model/main")
@CrossOrigin
public class MdmMainLineLossModelController {

	@Autowired
	IMdmAaLineLossModelService LineLossModelService;

	/**
	 * @param 5:母线不平衡; 6:主变线损
	 * @return
	 */
	@GetMapping("/search")
	public ResultInfo listModelListPage(VtMdmAaLineLossModel param, int pageNo, int pageSize, HttpServletRequest request) {
		...
		try {
			...
			if("4".equals(param.getSort())){
				resultInfo = new ResultInfo(LineLossModelService.queryMainLineModelList(param, pageNo, pageSize));
			}
			if("5".equals(param.getSort())){
				resultInfo = new ResultInfo(LineLossModelService.queryMainTransformerModelList(param, pageNo, pageSize));
			}
